...
class MonteCarloLib:
    ...
    def ml2p_iter2gc_mock(self, minus_log_prob_iter: List[Tuple[str, int, float]],
                     need_resort: bool = False, add1: bool = True) \
            -> List[Tuple[str, float, int, int, int, float]]:
        """
    ...
    def write_p(self, fd: TextIO):
        if not fd.writable():
            raise Exception(f"{fd.name} is not writable")
        if self.__gc is None:
            raise Exception(f"run mlps2gc before invoke this method")
        for pwd, mlp, appearance, rank, cracked, cracked_ratio in tqdm(self.__gc, desc="Saving: "):
            fd.write(f"{pwd}\t{mlp:.8f}\n")
        self.__gc = None
        pass
    
    def write_v(self, fd: TextIO):
        if not fd.writable():
            raise Exception(f"{fd.name} is not writable")
        if self.__gc is None:
            raise Exception(f"run mlps2gc before invoke this method")
        for pwd, _, mlp in tqdm(self.__gc, desc="Saving: "):
            fd.write(f"{pwd}\t{mlp}\n")
        self.__gc = None
        pass
