...
from lib4mc.nn_mia_MonteCarloParent import MonteCarlo
...
class NWordsMonteCarlo(MonteCarlo):
    ...
    def _structures(self, pwd, possible_list: List[float], container: List, probabilities: List, target_len: int,
                    iter_max: int, cur_iter: List[int], probabilities_list: List[List]):
        # 迭代深度过大，中断搜索
            ...
            if left in self.words:
                ...
                if len("".join([c for c in container if c != self.start_chr])) == target_len:
                    ...
                    if possible_list[0] > minus_log2_prob:
                        ...
                        probabilities_list[0] = copy.deepcopy(probabilities)
                    cur_iter[0] += 1
                self._structures(pwd[index:], possible_list, container, probabilities, target_len, iter_max, cur_iter, probabilities_list)
                container.pop()
                probabilities.pop()
    ...
    def calc_ml2p(self, pwd: str) -> Tuple[float, List[float]]:
        possible_list = [float(1022)]
        probabilities_list = [[]]
        depth = [0]
        self._structures(pwd + self.end_chr, possible_list, list(self.default_start),
                         [], len(pwd) + len(self.end_chr), MAX_ITER_DEPTH, depth, probabilities_list)
        return possible_list[0],probabilities_list[0]
...
def wrapper():
    ...
    for line in scored_testing:
        args.save.write(line[0]+'\t'+json.dumps(line[1])+'\n')
