	...
	while (true)
	{
		...
		else
		{
			...
			for (int i = 0;i < LetterTable.length;i++)
			{
				...
				// cout<<"Letter Table: " << LetterTable.str[i]<<endl;

				// cout<<"Letter Table length: " << LetterTable.length<<endl;
				if (p == 0)
					p = minL;
				score *= p/L;
			}
			// cout<<"Step 2: " << score<<endl;
			for (int i = 0;i < DigitTable.length;i++)
			{
				int p = DigitTree->find(DigitTable.str[i],strlen(DigitTable.str[i]));
				// cout<<"DigitTable Table: " << DigitTable.str[i]<<endl;
				if (p == 0)
					p = minD;
				score *= p/D;
			}
			for (int i = 0;i < SpecialTable.length;i++)
			{
				...
				// cout<<"SpecialTable: " << SpecialTable.str[i]<<endl;
				if (p == 0)
					p = minS;
				score *= p/S;
			}
			for (int i = 0;i < CharacterTable.length;i++)
			{
				...
				// cout<<"CharacterTable: " << CharacterTable.str[i]<<endl;
				if (p == 0)
					p = minC;
				score *= p/C;
			}
			// cout<<"Step 3: " << score<<endl;
        ...
        fileout <<password<<"\t"; //评分输出到文件中
		fileout <<score<<"\t"; //评分输出到文件中
		fileout << setiosflags(ios::scientific) << score << endl; //评分输出到文件中
	}
	/*
