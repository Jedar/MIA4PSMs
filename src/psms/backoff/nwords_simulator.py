...
class NWordsMonteCarlo(MonteCarlo):
    ...
    def _structures_log(self, pwd, possible_list: List[float], container: List, probabilities: List, target_len: int,
                    iter_max: int, cur_iter: List[int]):
        # 迭代深度过大，中断搜索
        if cur_iter[0] >= iter_max:
            return
        ...
        for index in range(1, len(pwd) + 1):
            ...
            if left in self.words:
                ...
                if len("".join([c for c in container if c != self.start_chr])) == target_len:
                    ...
                    if possible_list[0] > minus_log2_prob:
                        possible_list[0] = minus_log2_prob
                        possible_list[1] = [x for x in probabilities]
                    cur_iter[0] += 1
                self._structures_log(pwd[index:], possible_list, container, probabilities, target_len, iter_max, cur_iter)
                container.pop()
                probabilities.pop()
