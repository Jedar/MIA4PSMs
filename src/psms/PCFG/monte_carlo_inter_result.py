...
class MyScorer:
    ...
    def calc_prob(self, pwd: str):
        ...
        for s in structs:
            probs = []
            prob = 1.0
            v = self.count_base_structures.get(s, 0.0)
            probs.append(v)
            prob *= v
            ...
            for t in terminals:
                ...
                if tag == 'A':
                    v = self.count_alpha.get(len(pwd_part), {}).get(pwd_part.lower(), 0.0)
                    prob *= v 
                    probs.append(v)
                    ...
                    for p in pwd_part:
                        if p.isupper():
                            ...
                            alpha_mask += "L"
                    v = self.count_alpha_masks.get(len(alpha_mask), {}).get(alpha_mask, 0.0)
                    prob *= v 
                    probs.append(v)
                elif tag == 'O':
                    v = self.count_other.get(len(pwd_part), {}).get(pwd_part, 0.0)
                    prob *= v 
                    probs.append(v)
                elif tag == 'D':
                    v = self.count_digits.get(len(pwd_part), {}).get(pwd_part, 0.0)
                    prob *= v 
                    probs.append(v)
                elif tag == 'K':
                    v = self.count_keyboard.get(len(pwd_part), {}).get(pwd_part, 0.0)
                    prob *= v 
                    probs.append(v)
                elif tag == 'X':
                    v = self.count_context_sensitive.get(pwd_part, 0.0)
                    prob *= v 
                    probs.append(v)
                elif tag == 'Y':
                    v = self.count_years.get(pwd_part, 0.0)
                    prob *= v 
                    probs.append(v)
                else:
                ...
                pass
            if prob != 0:
                prob_list.append((prob, probs))
            pass
        if len(prob_list) == 0:
            return 0
        else:
            return max(prob_list, key=lambda x:x[0])

    def minus_log2_prob(self, pwd: str):
        prob, probs = self.calc_prob(pwd)
        return -log2(max(prob, self.minimal_prob)), [-log2(x) for x in probs]
    ...
    def calc_minus_log2_prob_from_file(self, passwords: TextIO) -> Dict[Any, Tuple[int, float]]:
        ...
        for pwd, num in raw_pwd_counter.items():
            prob, probs = self.minus_log2_prob(pwd)
            pwd_counter[pwd] = (num, prob, probs)
        return pwd_counter
        pass
...
def monte_carlo_wrapper(rule: str, target: TextIO, save2: TextIO, n: int = 100000):
    ...
    for pwd, info in sorted(scored_pwd_list.items(), key=lambda x: x[1][1], reverse=False):
        ...
        prev_rank = rank
        cracked += num
        save2.write(f"{pwd}\t{mlp:.8f}\t{num}\t{rank}\t{cracked}\t{cracked / total * 100:.2f}\t{','.join([str(x) for x in probs])}\n")
    save2.flush()
    save2.close()
