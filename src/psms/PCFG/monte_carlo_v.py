...
class MyScorer:
    ...
    def calc_prob(self, pwd: str) -> float:
        ...
        for s in structs:
            ...
            P.append(self.count_base_structures.get(s, 0.0))
            ...
            for t in terminals:
                ...
                if tag == 'A':
                    p = self.count_alpha.get(len(pwd_part), {}).get(pwd_part.lower(), 0.0)
                    prob *= p 
                    P.append(p)
                    ...
                    for p in pwd_part:
                        if p.isupper():
                            ...
                            alpha_mask += "L"
                    p = self.count_alpha_masks.get(len(alpha_mask), {}).get(alpha_mask, 0.0)
                    prob *= p
                    P.append(p)                    
                elif tag == 'O':
                    p = self.count_other.get(len(pwd_part), {}).get(pwd_part, 0.0)
                    prob *= p 
                    P.append(p)
                elif tag == 'D':
                    p = self.count_digits.get(len(pwd_part), {}).get(pwd_part, 0.0)
                    prob *= p 
                    P.append(p)
                elif tag == 'K':
                    p = self.count_keyboard.get(len(pwd_part), {}).get(pwd_part, 0.0)
                    prob *= p 
                    P.append(p)
                elif tag == 'X':
                    p = self.count_context_sensitive.get(pwd_part, 0.0)
                    prob *= p 
                    P.append(p)
                elif tag == 'Y':
                    p = self.count_years.get(pwd_part, 0.0)
                    prob *= p 
                    P.append(p)
                else:
...
def monte_carlo_wrapper(rule: str, target: TextIO, save2: TextIO, n: int = 100000):
    ...
    for pwd, info in sorted(scored_pwd_list.items(), key=lambda x: x[1][1][0], reverse=False):
        num, mlp = info
        save2.write(f"{pwd}\t{mlp[1]}\n")
